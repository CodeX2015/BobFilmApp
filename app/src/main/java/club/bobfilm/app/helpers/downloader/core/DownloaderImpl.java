...
import club.bobfilm.app.helpers.downloader.architecture.DownloadResponse;
...
import club.bobfilm.app.helpers.downloader.architecture.Downloader;
import club.bobfilm.app.helpers.downloader.db.DataBaseManager;
...
public class DownloaderImpl implements Downloader,
    ...
    public DownloaderImpl(DownloadRequest request, DownloadResponse response,
                          ...
                          String key, DownloadConfiguration config,
    ...
    private void init() {
        mDownloadInfo = new DownloadInfo(mRequest.getTitle().toString(),
                mRequest.getUri(), mRequest.getFolder());
    ...
    public void onDownloadCompleted() {
        ...
        //TODO Добавлено ветвление - проверяет число успешно завершенных,
        // завершенных с ошибкой и продолжающих скачку
        else {
            mDownloadInfo.setCompletedThreadsCount(mDownloadInfo.getCompletedThreadsCount() + 1);
            setFailedAndStillDownloadingThreadsCount();

            log.warn("DOWNLOADER onDownloadCompleted!!!\nCompleted Threads Count {}" +
                            "\nStill Downloading Threads Count {}\nFailed Threads Count {}",
                    mDownloadInfo.getCompletedThreadsCount(),
                    mDownloadInfo.getStillDownloadingThreadsCount(),
                    mDownloadInfo.getFailedThreadsCount());

            if (mDownloadInfo.getFailedThreadsCount() > 0
                    && mDownloadInfo.getStillDownloadingThreadsCount() == 0) {
                mDownloadInfo.setFailedThreadsCount(0);
                mDownloadInfo.setStillDownloadingThreadsCount(0);
                mDownloadInfo.setCompletedThreadsCount(0);
                log.warn("DOWNLOADER RESTART!!!!");
                restartFailedTasks();
            }
        }
    }
    ...
    public void onDownloadFailed(DownloadException de) {
        ...
        //TODO Добавлено ветвление - проверяет число завершенных с ошибкой и продолжающих скачку
        else {
            setFailedAndStillDownloadingThreadsCount();
            log.warn("DOWNLOADER onDownloadFailed!!! Failed Threads Count {} \n Still Downloading Threads Count {}",
                    mDownloadInfo.getFailedThreadsCount(), mDownloadInfo.getStillDownloadingThreadsCount());
        }
        //
    }
    ...
    //TODO --!> добавлен метод который находит число заврешенных с ошибкой и продолжающих закачку Task'ов
    private void setFailedAndStillDownloadingThreadsCount() {
        int failedCount = 0;
        int stillDownloadingCount = 0;
        for (DownloadTask task : mDownloadTasks) {
            if(task.isFailed()){
                failedCount++;
            }
            if (task.isDownloading()) {
                stillDownloadingCount++;
            }
        }
        mDownloadInfo.setFailedThreadsCount(failedCount);
        mDownloadInfo.setStillDownloadingThreadsCount(stillDownloadingCount);
    }
